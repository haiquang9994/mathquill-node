...
function MathQuill___() {
    /** jQuery from import jquery */
    var jQuery = __jQuery,
        undefined,
    ...
    Controller.open(function (_) {
        ...
        _.staticMathTextareaEvents = function () {
            ...
            this.container.prepend(jQuery('<span class="mq-selectable">').text("$" + ctrlr.exportLatex() + "$"));
            ctrlr.blurred = true;
            textarea
    ...
    LatexCmds.nrarr = LatexCmds.nrarr = LatexCmds.nrightarrow = bind(VanillaSymbol, "\\nrightarrow ", "&#8603;");
    ...
    var arrowsHtml =
        '<span class="mq-non-leaf mq-rightleftarrows"><svg width="1rem" height="1rem" viewBox="0 0 24 21" version="1.1" xmlns="http://www.w3.org/2000/svg"> <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <g transform="translate(0.823000, -0.000000)" fill="currentColor" fill-rule="nonzero"><path d="M5.403,-7.47669874e-14 L5.402,4 L23.177,4 L23.177,6 L5.402,6 L5.403,10.453 L0.177,5.226 L5.403,-7.47669874e-14 Z" transform="translate(11.677000, 5.226500) scale(-1, 1) translate(-11.677000, -5.226500)"></path><path d="M17.774,10.115 L23,15.341 L17.774,20.567 L17.774,16.115 L0,16.115 L0,14.115 L17.774,14.115 L17.774,10.115 Z" transform="translate(11.500000, 15.341000) scale(-1, 1) translate(-11.500000, -15.341000)"> </path></g></g></svg></span>';
    ...
    LatexCmds.sumSymbol = LatexCmds.summationSymbol = bind(
        VanillaSymbol,
        "\\summationSymbol",
        '<span class="mq-large-operator mq-non-leaf"><big>&sum;</big></span>'
    );
    ...
    LatexCmds.sumLower = LatexCmds.summationLower = bind(SummationLowerNotation, "\\summationLower", "&sum;");
    ...
    LatexCmds.sumUpper = LatexCmds.summationUpper = bind(SummationUpperNotation, "\\summationUpper", "&sum;");

    LatexCmds.prodSymbol = bind(
        VanillaSymbol,
        "\\prodSymbol",
        '<span class="mq-large-operator mq-non-leaf"><big>&prod;</big></span>'
    );
    ...
    LatexCmds.overleftrightarrow = P(MathCommand, function (_, super_) {
        _.init = function () {
            super_.init.call(
                this,
                "\\overleftrightarrow",
    ...
    var xRightArrow = (LatexCmds.xrightarrow = P(MathCommand, function (_, super_) {
    ...
    LatexCmds["\u222b"] = LatexCmds["int"] = LatexCmds.integral = P(SummationNotation, function (_, super_) {
        _.init = function () {
            var htmlTemplate =
    ...
    var Fraction = (LatexCmds.frac = LatexCmds.dfrac = LatexCmds.cfrac = LatexCmds.fraction = P(MathCommand, function (
        _,
        super_
    ) {
        _.ctrlSeq = "\\frac";
    ...
    LatexCmds.left = P(MathCommand, function (_) {
        _.parser = function () {
            ...
            return optWhitespace.then(regex(/^(?:[([|]|\\\{|\\langle\b|\\lVert\b)/)).then(function (ctrlSeq) {
    ...
    var Environment = P(MathCommand, function (_, super_) {
        ...
        _.wrappers = function () {
            return [_.template[0].join(this.environment), _.template[1].join(this.environment)];
        };
    });
    ...
    var Matrix = (Environments.matrix = P(Environment, function (_, super_) {
        ...
        _.html = function () {
            ...
            function parenHtml(paren) {
                return paren ? '<span class="mq-scaled mq-paren">' + paren + "</span>" : "";
            }
        ...
        _.createBlocks = function () {
            this.blocks = [MatrixCell(0, this), MatrixCell(0, this), MatrixCell(1, this), MatrixCell(1, this)];
        };
        _.parser = function () {
            var self = this;
            var optWhitespace = Parser.optWhitespace;
            ...
            return optWhitespace
                .then(string(delimiters.column).or(string(delimiters.row)).or(latexMathParser.block))
                .many()
                .skip(optWhitespace)
                .then(function (items) {
                    var blocks = [];
        ...
        _.addRow = function (afterCell) {
            ...
            for (var i = 0; i < columns; i += 1) {
                ...
                block.jQ = $('<td class="mq-empty">').attr(mqBlockId, block.id).appendTo(newRow);
            }
        ...
        _.backspace = function (cell, dir, cursor, finalDeleteCallback) {
            ...
            if (cell.isEmpty()) {
                ...
                while (dirwards && dirwards[dir] && this.blocks.indexOf(dirwards) === -1) {
    ...
    LatexCmds["\u2217"] = LatexCmds.ast = LatexCmds.star = LatexCmds.loast = LatexCmds.lowast = bind(
        BinaryOperator,
        "\\ast ",
        "&lowast;"
    );
    ...
    LatexCmds.nharr = LatexCmds.nlrarr = LatexCmds.nleftrightarrow = bind(
        VanillaSymbol,
        "\\nleftrightarrow ",
        "&#8622;"
    );
    ...
    LatexCmds.sigmaf = LatexCmds.sigmav = LatexCmds.varsigma = bind(Variable, "\\varsigma ", "&sigmaf;"); //W3C/Unicode //Elsevier //LaTeX

    LatexCmds.thetav = LatexCmds.vartheta = LatexCmds.thetasym = bind(Variable, "\\vartheta ", "&thetasym;"); //Elsevier and 9573-13 //AMS and LaTeX //W3C/Unicode
    ...
    LatexCmds.Alpha = LatexCmds.Epsilon = LatexCmds.Zeta = LatexCmds.Eta = LatexCmds.Iota = LatexCmds.Kappa = LatexCmds.Rho = LatexCmds.Tau = LatexCmds.Beta = LatexCmds.Chi = LatexCmds.Gamma = LatexCmds.Delta = LatexCmds.Theta = LatexCmds.Lambda = LatexCmds.Xi = LatexCmds.Pi = LatexCmds.Sigma = LatexCmds.Phi = LatexCmds.Psi = LatexCmds.Omega = LatexCmds.forall = P(
        VanillaSymbol,
        function (_, super_) {
            _.init = function (latex) {
                super_.init.call(this, "\\" + latex + " ", "&" + latex + ";");
